...
class CDEvaluator():
    ...
    def _collect_running_batch_states(self,args):
        ...
        # vis1 = np.concatenate([vis_input])
        # vis2 = np.concatenate([vis_gt])
        # vis = np.clip(vis, a_min=0.0, a_max=1.0)
        # vis1 = np.clip(vis1, a_min=0.0, a_max=1.0)
        # vis2 = np.clip(vis2, a_min=0.0, a_max=1.0)#限制数组中的值，也就是说clip这个函数将数组中的元素限制在a_min, a_max之间，大于a_max的就使得它等于 a_max，小于a_min,的就使得它等于a_min
        # file_name = os.path.join(
        #         self.vis_dir, 'eval_pred' + str(self.batch_id)+'.jpg')
        # plt.imsave(file_name, vis)
        # file_name = os.path.join(
        #     self.vis_dir, 'eval_A' + str(self.batch_id) + '.jpg')
        # plt.imsave(file_name, vis1)
        # file_name = os.path.join(
        #     self.vis_dir, 'eval_L' + str(self.batch_id) + '.jpg')
        # plt.imsave(file_name, vis2)
    ...
    def  _forward_pass(self,batch,args):
        ...
        img_in12 = torch.abs(img_in1 - img_in2)
        ...
        if args.loss_SD== True :
            ...
            self.G_pred ,x1,x2,x3,x4,catx1,catx2,t12,d12= self.net_G(img_in1, img_in2,img_in1, img_in2,img_in12)
