...
class SEIFNet(Backbone):
    ...
    def forward(self, x1, x2,x3, x4,DL12):


        #res18
        f1 = self.backbone(x1)
        f2 = self.backbone(x2)
        fdl12 = self.backbone(DL12)

        x1_0,x1_1,x1_2,x1_3 = f1
        x2_0,x2_1,x2_2,x2_3 = f2
        xdl12_0,xdl12_1,xdl12_2,xdl12_3 = fdl12

        tdl12 = torch.abs(x1_1 - x2_1)

        xdl1_2_1 = self.avgpool(xdl12_1)
        xdl1_2_1 = xdl1_2_1.view(xdl1_2_1.size(0), -1)
        xdl1_2_1 = self.proj2(xdl1_2_1)
        

        tdl1_2 = self.avgpool(tdl12)
        tdl1_2 = tdl1_2.view(tdl1_2.size(0), -1)
        tdl1_2 = self.proj2(tdl1_2)

        f3 = self.backbone(x3)
        f4 = self.backbone(x4)

        x3_0,x3_1,x3_2,x3_3 = f3
        x4_0,x4_1,x4_2,x4_3 = f4
        


        # catx = torch.cat((x_1, x_2), dim=0)
        x13 = self.avgpool(x1_3)
        x13 = x13.view(x13.size(0), -1)
        x13 = self.proj(x13)

        x23 = self.avgpool(x2_3)
        x23 = x23.view(x23.size(0), -1)
        x23 = self.proj(x23)

        x33 = self.avgpool(x3_3)
        x33 = x33.view(x33.size(0), -1)
        x33 = self.proj(x33)
        ...
        catx1 = torch.cat((x1_3, x2_3), dim=0)
        catx1 = self.avgpool(catx1)
        catx1 = catx1.view(catx1.size(0), -1)
        catx1 = self.proj(catx1)

        catx2 = torch.cat((x3_3, x4_3), dim=0)
        catx2 = self.avgpool(catx2)
        catx2 = catx2.view(catx2.size(0), -1)
        catx2 = self.proj(catx2)
        ...
        map3_3 = output.cpu()
        feature_map_data_list = [map3_3[0, i].detach().numpy() for i in range(map3_3.shape[1])]
...
# 可视化每个特征图的热力图
        ...
        #     plt.axis('off')
        #     plt.savefig(f"/home/hdda/liangyizhou/mySEIFNetcopy/vis/FeatureMap{i + 1}")
        # plt.show()



        return output,x13,x23,x33,x43,catx1,catx2,xdl1_2_1,tdl1_2
